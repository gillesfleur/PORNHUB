...
import { Play, Eye, ThumbsUp, Clock, Info, X, Menu, Trash2 } from 'lucide-react';
...
export const VideoCard: React.FC<VideoCardProps> = ({ 
  ...
  const cardVariants = {
    ...
    visible: (i: number) => ({
      ...
      y: 0,
      transition: {
        delay: (i % 8) * 0.05, // Staggered delay based on index (reset every 8 for rows)
        duration: 0.4,
        ease: 'easeOut'
      }
    })
  ...
  if (variant === 'horizontal') {
    return (
      <motion.div 
        ...
        custom={index}
        className="group flex items-center gap-4 cursor-pointer hover:bg-surface/40 p-2 rounded-xl transition-colors relative"
      >
        {showDragHandle && (
          <div className="flex items-center gap-3 text-muted/40 group-hover:text-muted transition-colors">
            <div className="cursor-grab active:cursor-grabbing">
              <Menu size={18} />
            </div>
            {index !== undefined && (
              <span className="text-sm font-black w-4 text-center">{index + 1}</span>
            )}
          </div>
        )}
        ...
        <Link 
          to={`/video/${slug}`}
          className="flex flex-grow gap-4 min-w-0"
        >
        {/* Thumbnail Container */}
